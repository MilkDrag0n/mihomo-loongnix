...
`THEME-001`：按钮、输入框、下拉框、弹窗和表格操作格均使用黑/灰中性背景，不得出现蓝色背景。

## 2. 首页

```text
首页
├─ 实际运行状态（GET /v1/status）
...
- `HOME-002`：伪造 service active、控制接口失败，必须显示“服务活动但控制接口异常”，不能显示运行。
- `HOME-003`：启动成功后核对 systemd、9090 和页面三处一致。
- `HOME-004`：快速连续点击启动，只产生一个在途请求。
- `HOME-005`：TUN 配置为真但内核关闭，显示“已预设，尚未实际生效”。
